...
with open(exon_file,'w') as f:
    f.write('\t'.join(['ID','proteinName','chromosome','exonStart','exonEnd','phase','strand'])+'\n') #write the header to the csv
    for _,row in amplicon_positions.iterrows():
        ...
        if attributes['ID'] not in already_done:
            ...
            for _,exon in exons_this_gene.iterrows():
                f.write('\t'.join([attributes['ID'], attributes['Name'], row.chrom, str(exon.start),str(exon.end),str(exon.phase),str(exon.strand)])+'\n')
