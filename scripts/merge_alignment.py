...
combines reads based on their alignments

usage: python merge_alignment.py [unmerged_alignment.bam] [outfile.fastq]
"""
...
#set up variables to hold pairs, new sequences and statistics
reads_paired = []
new_seqs = []
unpaired = 0
merged_total = 0
...
for read1, read2 in read_pair_generator(samfile): # loop through each pair in the samfile
    if read2 is None: #unmerged pair
        ...
        if len(overlap) != 0: # the reads overlap
            #extract overlaps and unique parts for read1
            seq_pos1 = [i for i,a in enumerate(read_1_aligned) if a in overlap] #overlapping sequence positions in read1
            seq_1_only = read1.query_alignment_sequence[:min(seq_pos1)] #sequence unique to read1
            overlap_1 = read1.query_alignment_sequence[min(seq_pos1):max(seq_pos1)] #overlapping sequence
            quals_1 = read1.query_qualities[read1.query_alignment_start:read1.query_alignment_end] #quals of read1
            quals_1_only = quals_1[:min(seq_pos1)] #quals of read1 only
            quals_1_overlap = quals_1[min(seq_pos1):max(seq_pos1)] #quals in overlapping sequence
            ...
            if overlap_1 == overlap_2: #if the overalps are the same, nothing needs to be done
                new_overlap = overlap_1
                new_quals_overlap = [max(a,b) for a,b in zip(quals_1_overlap,quals_2_overlap)] #keep the maximum score 
            else: #if the overlaps are not the same, always take the base with the highest quality score
                new_overlap = ''
                new_quals_overlap = []
            ...
            #combine into new sequence & quals
            ...
            not_merged_total += 2
#print some stats for logging purposes
...
with open(outfile,'w') as f:
    for i in new_seqs:
        try:
            f.write(f'@{i[2]}\n{i[0]}\n+\n{"".join([chr(round(j+33)) for j in i[1]])}\n') #quality score = phred encoding (ascii of value+33)
        except TypeError:
            print(i)
            raise
